...
ITERATIONS = 20000				# number of times radamsa is called to mutate the seed file
MAX_FILE_SIZE = 3000				# radamsa-generated files larger than this will be discarded
...
SEED_FILE_PATH = "./LAVA-M_utmp_2k_sample.bin"	# path to seed file
FUZZER_DIR_PATH = "/ilab/users/jtd124/fuzzing/AFL/"		# AFL directory
TARGET_PATH = "./LAVA-M_who"
...
def main():
    ...
    for i in range(1, ITERATIONS):
        ...
        bytes = mutate()
        if len(bytes) > MAX_FILE_SIZE:					# discard files over 5k
            continue
        filename = hashlib.md5(bytes).hexdigest() + ".utmp"	# only unique files will be saved
